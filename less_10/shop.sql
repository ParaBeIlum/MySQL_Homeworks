...
INSERT INTO `account` VALUES (1,'first','qwerty'),(2,'second','qwerty'),(3,'third','qwerty');
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Разделы интернет-магазина';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `catalogs` VALUES (1,'Процессоры'),(2,'Мат.платы'),(3,'Видеокарты'),(4,'Аудиокарты');
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER log_catalogs AFTER INSERT ON catalogs
FOR EACH ROW
BEGIN
  INSERT INTO logs (created_at, name, id, name_value)
...
CREATE TABLE `logs` (
  ...
  `id` int(11) NOT NULL,
  `name_value` varchar(45) COLLATE utf8_unicode_ci NOT NULL
...
INSERT INTO `logs` VALUES ('2020-08-04 16:32:08','users',8,'Константин'),('2020-08-04 16:34:23','catalogs',4,'Аудиокарты'),('2020-08-04 16:37:46','products',10,'Некоторая аудиокарта');
...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Товарные позиции';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `products` VALUES (1,'Intel Core i3-8100','',7890.00,1,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(2,'Intel Core i5-7400','',12700.00,1,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(3,'AMD FX-8320E','',4780.00,1,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(4,'AMD FX-8320','',7120.00,1,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(5,'ASUS ROG MAXIMUS X HERO','',19310.00,2,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(6,'Gigabyte H310M S2H','',4790.00,2,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(7,'MSI B250M GAMING PRO','',5060.00,2,'2020-06-30 00:00:31','2020-06-30 00:01:06'),(8,'test',NULL,1.00,1,'2020-08-03 16:03:17','2020-08-03 16:03:17'),(9,'test','test',2.00,2,'2020-08-03 16:04:29','2020-08-03 16:04:29'),(10,'Некоторая аудиокарта','',1000.00,4,'2020-08-04 16:37:46','2020-08-04 16:37:46');
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER log_products AFTER INSERT ON products
FOR EACH ROW
BEGIN
  INSERT INTO logs (created_at, name, id, name_value)
...
INSERT INTO `test_tbl` VALUES ('2018-08-01'),('2016-08-04'),('2018-08-16'),('2018-08-17');
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER log_users AFTER INSERT ON users
FOR EACH ROW
BEGIN
  INSERT INTO logs (created_at, name, id, name_value)
...
/*!50001 VIEW `new_view` AS select `p`.`id` AS `product_id`,`p`.`name` AS `product_name`,`cat`.`name` AS `cat_name` from (`products` `p` left join `catalogs` `cat` on((`p`.`catalog_id` = `cat`.`id`))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `test` AS select `account`.`id` AS `user_id`,`account`.`name` AS `user_name` from `account` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-08-12  1:59:36
